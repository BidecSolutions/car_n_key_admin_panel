...
import { trimAPI } from "../services/api";
import {
  DEFAULT_PAGE_SIZE,
  DEFAULT_CURRENT_PAGE,
} from "../utils/constants";
import { useToast } from "../utils/common/ToastProvider";
import TrimList from "../components/Trim/TrimList";
import TrimForm from "../components/Trim/TrimForm";
...
const Trim = () => {
  ...
  const fetchfeature = async () => {
    try {
      setLoading(true);
      const response = await trimAPI.getAll();
  ...
  const handleDeleteCode = async (id, action = "delete") => {
    try {
      await trimAPI.toggle_status(id, action);
      await fetchfeature();
      toast.success(
        action === "delete" ? "Trim deleted!" : "Trim restored!"
      );
...
const handleFormSubmit = async (formData) => {
  try {
    ...
    if (editingCode) {
      await trimAPI.update(editingCode.id, formData);
    } else {
      await trimAPI.create(formData);
    }
  ...
  return (
    <div>
      ...
      {/* Modal Form */}
      <Modal
        title={editingCode ? "Edit Trim" : "Add New Trim"}
        open={modalVisible}
