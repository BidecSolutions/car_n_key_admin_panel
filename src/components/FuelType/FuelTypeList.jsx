    ...
    try {
      ...
      message.success(
        action === "delete"
          ...
          : "Fuel Type restored successfully!"
      );
    } catch (error) {
      message.error(
        action === "delete"
          ? "Failed to delete Fuel Type. Please try again."
          : "Failed to restore Fuel Type. Please try again."
      );
...
const columns = [
  ...
  return (
    <Table
      columns={columns}
      dataSource={Array.isArray(fuelType) ? fuelType : []}
      rowKey="id"
      loading={loading}
      pagination={{
        ...pagination,
        ...
        showTotal: (total, range) =>
          `${range[0]}-${range[1]} of ${total} Fuel Type`,
      }}
